...
function getActualVersion() {
  ...
  xhtr.onload = () => {
    ...
    if (app.getVersion().toString() !== r.tag_name) {
      ...
      .text(
        `<a href="http://soube.diegomolina.cl">${config().langFile[config().lang].alerts.newVersion}</a>`
      );
...
function loadSongs() {
  if (Object.keys(songsFile()).length === 0) {
    getActualVersion();
    $('#list-songs').text(
      `<div id="init-message">${config().langFile[config().lang].alerts.welcome}</div>`
    );
  } else {
    checkNewSongs();
