...
module.exports = function PlayFile () {
  // Funciones
  ...
  function play () {
    ...
    xhtr.onload = () => {
      audioContext.decodeAudioData(xhtr.response).then(buffer => {
        ...
        // source.connect(filter[0]).connect(audioContext.destination)
  ...
  // function setFilterVal (a, b) { filter[a].gain.value = b }

  // Crea y asigna BiquadFilter de tipo peaking por cada frequencia
  // [50, 100, 156, 220, 331, 440, 622, 880, 1250, 1750, 2500, 3500, 5000, 10000, 20000]
  // function filters () {
  //   let _hrzGain = jread(__CONFIG_FILE).config.equalizer
